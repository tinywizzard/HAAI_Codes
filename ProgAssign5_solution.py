...
def mean_overall_country(df, country):
    #Write code here
    return df["Overall"].loc[df["Nationality"].str.contains(fr"^{re.escape(country)}$", flags=re.IGNORECASE)].mean()
...
def common_players(df, club, country):
    #Write code here
    common_players = df["Name"].loc[df["Nationality"].str.contains(fr"^{re.escape(country)}$", flags=re.IGNORECASE) &
                                    df["Club"].str.contains(fr"^{re.escape(club)}$", flags=re.IGNORECASE)]
